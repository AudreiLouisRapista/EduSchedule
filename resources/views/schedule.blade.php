              ...
              <div class="card-header" style="background:#fff;">
                <h3 class="card-title">Schedule list</h3>
                    <!-- Button trigger modal -->
                              ...
                              <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" style="float: right; border-radius: 20px; background-color: #4CAF50; border: none; padding: 10px 20px; font-size: 16px; cursor: pointer; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2); transition: background-color 0.3s ease;">
                             Add Schedule
                            </button>

                            <!-- Modal -->
                          <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="addScheduleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                                <div class="modal-content" style="border-radius: 20px;">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <form method="POST" action="{{ route('save_schedule') }}" enctype="multipart/form-data">
                                                            @csrf
                                                            ...
                                                            <div class="mb-3">
                                                                ...
                                                                @error('teachers_id') <span class="text-danger small">{{ $message }}</span> @enderror
                                                            </div>
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <div style="display: flex; flex-wrap: wrap; gap: 10px;">
                                                                @php
                                                                    $days = ['Whole Week','Monday','Tuesday','Wednesday','Thursday','Friday'];
                                                                @endphp
                                                        ...
                                                        function handleDaySelection(el) {
                                                            ...
                                                            <div class="row mb-3">
                                                                <div class="col-md-6">
                                                                    ...
                                                                    <input type="time" class="form-control" id="sub_Stime" name="sub_Stime" required>
                                                                    @error('sub_Stime') <span class="text-danger small">{{ $message }}</span> @enderror
                                                                </div>
                                                                <div class="col-md-6">
                                                                    <label for="sub_Etime" class="form-label">End Time</label>
                                                                    <input type="time" class="form-control" id="sub_Etime" name="sub_Etime" required>
                                                                    @error('sub_Etime') <span class="text-danger small">{{ $message }}</span> @enderror
                                                                </div>
                                                            </div>
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        @error('sched_year') <span class="text-danger small">{{ $message }}</span> @enderror
                                                    </div>
               ...
               <table id="example2" class="table table-bordered table-hover">
                            ...
                            <tbody>
                              @foreach ($view_schedule as $sched)
                                ...
                                <div class="modal fade" id="deleteScheduleModal{{ $sched->schedule_id }}" tabindex="-1" role="dialog" aria-labelledby="deleteScheduleModal{{  $sched->schedule_id}}" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="deleteScheduleModal{{  $sched->schedule_id }}">Delete Schedule</h5>
                                ...
                                <div class="modal fade" id="UpdateScheduleModal{{ $sched->schedule_id }}" tabindex="-1" role="dialog" aria-labelledby="UpdateScheduleModal{{ $sched->schedule_id}}" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered modal-lg">
                                        <div class="modal-content" style="border-radius: 20px;">
                                            <div class="modal-header">
                                            ...
                                            <div class="modal-body">
                                                <form method="POST" action="{{ route('update_schedule') }}">
                                                    ...
                                                    <div class="mb-3">
                                                        <label for="teachers_id" class="form-label">Assigned Teacher</label>
                                                        <select class="form-select" id="teachers_id" name="teachers_id">
                                                            <option value="{{ $sched->teachers_id }}">{{ $sched->teacher_name ?? 'Not Assigned' }}</option>
                                                            <option value="0">Not Assigned</option>
                                                            @foreach($teachers as $teacher)
                                                                <option value="{{ $teacher->teachers_id }}">{{ $teacher->name }}</option>
                                                            @endforeach
                                                        </select>
                                                        @error('teachers_id') <span class="text-danger small">{{ $message }}</span> @enderror
                                                    </div>
                                                    ...
                                                    <div class="mb-3">
                                                        <label for="subject_id" class="form-label">Assigned Subject</label>
                                                        <select class="form-select" id="subject_id" name="subject_id" required>
                                                            <option value="{{ $sched->subject_id }}">{{ $sched->sub_name }}</option>
                                                            @foreach($subject as $sub)
                                                    ...
                                                    <div class="row mb-3">
                                                        <div class="col-md-6">
                                                            <label for="sub_Stime" class="form-label">Starting Time</label>
                                                            <input type="time" class="form-control" id="sub_Stime" name="sub_Stime" required value="{{ $sched->sub_Stime }}">
                                                            @error('sub_Stime') <span class="text-danger small">{{ $message }}</span> @enderror
                                                        </div>
                                                        <div class="col-md-6">
                                                            <label for="sub_Etime" class="form-label">End Time</label>
                                                            <input type="time" class="form-control" id="sub_Etime" name="sub_Etime" required value="{{ $sched->sub_Etime }}">
                                                            @error('sub_Etime') <span class="text-danger small">{{ $message }}</span> @enderror
                                                        </div>
                                                    </div>
